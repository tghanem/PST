...
namespace pst.utilities
{
    public static class Crc32
    {
        ...
        public static uint ComputeCrc32(byte[] data)
        {
            ...
            {
                result ^= BitConverter.ToUInt32(data, i * 8);

                result = CrcTableOffset88[result & 0x000000FF] ^
                         CrcTableOffset80[(result >> 8) & 0x000000FF] ^
                         CrcTableOffset72[(result >> 16) & 0x000000FF] ^
                         CrcTableOffset64[(result >> 24) & 0x000000FF];

                var dword2 = BitConverter.ToUInt32(data, i * 8 + 4);

                result ^= CrcTableOffset56[dword2 & 0x000000FF] ^
                          CrcTableOffset48[(dword2 >> 8) & 0x000000FF] ^
                          CrcTableOffset40[(dword2 >> 16) & 0x000000FF] ^
                          CrcTableOffset32[(dword2 >> 24) & 0x000000FF];
            }

            var endUnalignedBytesOffset = (data.Length / 8) * 8;
